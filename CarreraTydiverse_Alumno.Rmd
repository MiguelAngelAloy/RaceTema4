...
```{r}
cran2 %>%
  arrange(ip_id, country, desc(r_version))
...
students3_tidy <- students3 %>%
  gather(key = "class_exam", value = "score", -name) %>% 
  na.omit()
students3_tidy
```
...
students3_tidy <- students3_tidy %>%
  mutate(class = parse_number("class5"))
students3_tidy
```
...
wday_part <- wday(this_day, label = TRUE) 
cat("Year:", year_part, "\n")
cat("Month:", month_part, "\n")
cat("Day:", day_part, "\n")
cat("Weekday:", wday_part, "\n")
...
cat("Hour:", hour_part, "\n")
cat("Minute:", minute_part, "\n")
cat("Second:", second_part, "\n")
...
cat("Clase de my_date:", class(my_date), "\n")
cat("my_date2:", my_date2, "\n")
cat("my_date3:", my_date3, "\n")
...
datetime1 <- ymd_hms(dt1)
cat("Datetime1:", datetime1, "\n")
...
```{r}
dt2 <- c("2014-08-23", "2015-09-15", "2016-12-01", "2017-03-18")
dates_vector <- ymd(dt2)
cat("Dates Vector:", dates_vector, "\n")
...
```{r}
this_moment <- now()
cat("Instante original:", this_moment, "\n")
this_moment_updated <- c(hours = hour(this_moment),minutes = minute(this_moment), seconds = second(this_moment))
this_moment_updated
...
llegada_nyc <- salida + minutes(15*60 + 50)
